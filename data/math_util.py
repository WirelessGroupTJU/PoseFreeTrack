...
def rotate_data(data, ori_q=None, ori=None):
    ori_q = quaternion.from_float_array(ori) if ori_q is None else ori_q
    data_q = quaternion.from_float_array(np.concatenate([np.zeros([data.shape[0], 1]), data], axis=1))
    return quaternion.as_float_array(ori_q * data_q * ori_q.conj())[:, 1:]
