...
from .modules import CModel, RNNModel, PRNNModel, Discriminator, StackEncoder, noise, Swapaxes, Newaxes, ExtraNone
import numpy as np

class TransPoseMT(nn.Module):
    def __init__(self, name, hyperparams, domain_num, hidden_size, proj_size, num_layers, dropout=0.05, adv_loss='js', label_type='polar', label_mode='sparse', loss_func='mse', trans_func=None, **kwargs):
        ...
        self.gen = self.get_generator(self.hps['gen_type'], self.hps['gen_flat'], self.hps['gen_bidirect'])
        if self.hps['dec_type'] != 'share':
            self.dec = self.get_generator(self.hps['dec_type'], self.hps['dec_flat'], self.hps['dec_bidirect'])
        else:
            self.dec = self.gen
    ...
    def get_encoder(self):
        if self.hps['enc_flat']:
            input_size = self.feature_size if not self.hps['cond_enc'] else self.feature_size + self.domain_num
            enc_fuse = self.get_rnn(self.hps['enc_type'], input_size, self.hidden_size, self.num_layers[0], self.hps['enc_bidirect'], self.proj_size, self.dropout, True, self.hps['activation'], False, False, True)
            if self.hps['cond_enc']:
                encoder = CModel(enc_fuse, self.domain_embedding)
            else:
                encoder = enc_fuse
        else:
            ...
            if self.hps['cond_enc']:
                enc_gyro = CModel(enc_gyro, self.domain_embedding)
                enc_acce = CModel(enc_acce, self.domain_embedding)
            enc_fuse = self.get_rnn(self.hps['enc_type'], enc_gyro.rnn_size+enc_acce.rnn_size, self.hidden_size, self.num_layers[0], self.hps['enc_bidirect'], self.proj_size, self.dropout, True, self.hps['activation'], False, False, True)
            encoder = StackEncoder(enc_gyro, enc_acce, enc_fuse, self.hps['cond_enc'], self.gys_feature_size)
        self.deepz_size = enc_fuse.rnn_size
        return ExtraNone(encoder) if self.hps['enc_flat'] else encoder
    
    def get_generator(self, gen_type, is_flat, bidirectional):
        input_size = self.deepz_size+self.domain_num
        proj_size = [self.gys_feature_size, self.acc_feature_size] if not is_flat else self.gys_feature_size + self.acc_feature_size
        dec = self.get_rnn(gen_type, input_size, self.hidden_size, self.num_layers[0], bidirectional, proj_size, self.dropout, True, None, False, False, is_flat)
        return CModel(dec, self.domain_embedding)
        
    def get_discriminator(self):
        if not self.hps['dis_flat']:
            raise NotImplementedError('dis_flat has to be false')
    ...
    def get_predictor(self):
        proj_size = [1] * self.label_dim if not self.hps['pred_flat'] else self.label_dim
        return self.get_rnn(self.hps['pred_type'], self.deepz_size, self.hidden_size, self.num_layers[2], self.hps['pred_bidirect'], proj_size, self.dropout, True, None, False, self.hps['pred_linear'], self.hps['pred_flat'])
